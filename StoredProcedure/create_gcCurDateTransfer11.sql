...
IF EXISTS ( SELECT * 
            FROM   sysobjects 
            WHERE  id = object_id(N'[dbo].[gcCurDateTransfer11]') 
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
BEGIN
    DROP PROCEDURE [dbo].[gcCurDateTransfer11]
END
...
BEGIN
	...
	FROM [dbo].[PlcCur11] ORDER BY dt DESC
	...
	EXECUTE [dbo].[gcUpdateCurData] @dt = @dt, @ID = 198, @Value = @FE_7303_04_F02_norm_Q  
 	...
 	DELETE FROM [dbo].[PlcCur11] WHERE dt<=@dt
