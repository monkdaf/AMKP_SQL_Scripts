...
IF EXISTS ( SELECT * 
            FROM   sysobjects 
            WHERE  id = object_id(N'[dbo].[gcCurDateTransfer09]') 
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
BEGIN
    DROP PROCEDURE [dbo].[gcCurDateTransfer09]
END
...
BEGIN
	...
	FROM [dbo].[PlcCur09] ORDER BY dt DESC
	...
	EXECUTE [dbo].[gcUpdateCurData] @dt = @dt, @ID = 170, @Value = @FE3_12_norm_Q
 	...
 	DELETE FROM [dbo].[PlcCur09] WHERE dt<=@dt
